...
router.post("/signUp", async (req, res) => {
  try {
    ...
    res.send(
      "Please check your inbox and verify your email id. Please login back again using your credentials"
    );
...
router.get("/verify/:id/:token", async (req, res) => {
  try {
    ...
    if (!user) {
      res.send(
        `User not exists. Please <a href="http://localhost:3000/user/signUp">Sign Up</a> here`
      );
    }
...
router.post("/reset", async (req, res) => {
  ...
  res.send(
    "please check your email and reset your password on clicking the link provided <br/><br/><a href='/user/signin'>Login here</a>"
  );
});
...
router.get("/logout", async (req, res) => {
  ...
  req.session.save(function (err) {
    ...
    req.session.regenerate(function (err) {
      ...
      res.send(
        `logout successful <br/><br/><a href='/user/signin'>Login here</a>`
      );
    });
  });
